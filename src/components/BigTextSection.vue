<template>
  <SectionPrototype v-intersect="runAnimation" verticalTitle="" ref="textContainer" :className="`big-text-section ${className} ${centered ? 'centred text-center' : ''} ${noAnimated ? 'no-animation' : ''}`" container-class-name="big-text-container" :theme="theme || 'dark'">
    <h2 :class="`big-text events-none invisible ${centered ? 'w-100' : ''}`" ref="fixedText" v-html="text"></h2>
    <h2 :class="`big-text events-none position-absolute ${centered ? 'w-100' : ''}`" ref="animatedText" v-html="text"></h2>
    <slot/>
...
.big-text-section {
  ...
  @media (max-width: $media-sm) {
    ...
    .line-bg-type1 {
      ...
      background-position: left bottom;
      background-image: url("data:image/svg+xml,%3Csvg width=%271420%27 height=%271681%27 xmlns=%27http://www.w3.org/2000/svg%27%3E%3Cg transform=%27rotate%28-11 710 841%29%27%3E%3Cpath d=%27M-145.255 1530.392s16.998-201.252 233.496-190.882c216.499 10.37 403.605 178.568 555.613 166.378 152.008-12.19-11.623-271.287 178.313-369.588 189.936-98.302 353.99-139.321 273.233-355.41-80.758-216.089 212.753-608.741 470.658-628.502%27 fill=%27none%27 stroke=%27%2372fdda%27 stroke-miterlimit=%2720%27 stroke-width=%272%27/%3E%3C/g%3E%3C/svg%3E");
      width: 34vw;
      height: 10vw;
    }
    .line-bg-type2 {
